...
- **CRL URLs (GitHub Pages via the `owncloud.dev` org domain):**
  - leaf/developer CRL: `https://owncloud.dev/developer-certificates/crl/developers.crl`
  - intermediate CRL: `https://owncloud.dev/developer-certificates/crl/intermediate.crl`
  - root CRL: `https://owncloud.dev/developer-certificates/crl/root.crl`

  These are the constants the verifier is built with (§9, §13). The org user-site
  uses a Pages custom domain (`owncloud.dev`), so the `owncloud.github.io/*` path
  301-redirects org-wide and cannot be used directly by the no-redirect verifier;
  the CRL is served at `owncloud.dev` instead (§13).
...
- **GitHub Pages served via the `owncloud.dev` org domain:**
  `https://owncloud.dev/developer-certificates/crl/` (leaf = `developers.crl`,
  plus `intermediate.crl`, `root.crl`). The leaf CRL is generated by the ledger
  workflow (`crlgen`) and committed to the repo root `crl/`; a separate Pages
  deploy workflow (`.github/workflows/pages.yml`) publishes it.
- **Why `owncloud.dev`, not the bare `github.io` URL:** the ownCloud org user-site
  (`owncloud/owncloud.github.io`) already uses a Pages **custom domain**
  (`owncloud.dev`). GitHub therefore 301-redirects **every** `owncloud.github.io/*`
  path to `owncloud.dev/*` org-wide — this is not a per-repo setting and cannot be
  turned off for this repo. Since the verifier uses a **core-side constant URL** and
  does **not** follow redirects (§9), the constant must be the `owncloud.dev` URL
  that returns 200 directly. (The original design assumed a bare-`github.io`
  Option B; that premise does not hold for an org with a user-site custom domain.)
- **Only the CRL is published — not the repo.** Pages uses the **GitHub Actions**
  build (not "deploy from a branch"): `pages.yml` stages only `crl/*.crl` into the
  artifact, so the Pages site contains nothing but the CRL(s). No repository source,
  docs, or ledger is exposed on the web.
- **Future migration to a dedicated domain:** a **code change to a new URL constant
  plus independent hosting** — NOT by moving/removing the org user-site custom
  domain (which would break every other owncloud project page). The existing
  `owncloud.dev` CRL URL must keep returning 200 so already-deployed instances
  continue to work.
...
- **Operational identifiers: decided** — repo `owncloud/developer-certificates`;
  CRL URLs under `https://owncloud.dev/developer-certificates/crl/`
  (`developers.crl`, `intermediate.crl`, `root.crl`) — served via `owncloud.dev`
  because the org user-site custom domain 301-redirects the `github.io` path
  (§13). See the header block.
