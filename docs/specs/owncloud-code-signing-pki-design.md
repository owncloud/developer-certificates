...
**Write path (bot-writes-via-PR).** `main` is protected — it requires pull
requests and verified signatures — so the bot does **not** push commits or use
the Contents API directly. Every ledger write goes through an **auto-merged,
App-signed pull request**: the bot commits the file on a short-lived branch via
the Git Data API (GitHub signs the commit under the App identity, satisfying
`required_signatures`), opens a PR, and merges it once the required `validate`
check reports success. The single-concurrency serialization below is preserved
because each bot run holds the `ledger-write` concurrency group for its whole
duration and the propose call blocks until the PR merges. See
[OPERATIONS.md](../OPERATIONS.md) for the App/ruleset configuration.
...
- **Start with GitHub Pages, default `github.io` URL (Option B):**
  ...
  the ledger workflow, published into the repo **via the same auto-merged,
  App-signed PR path as the ledger** (§6 — `main` forbids direct pushes), and
  served over the Pages/Fastly CDN. Maximally co-located with the ledger.
